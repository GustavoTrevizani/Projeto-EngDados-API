...
tmp_parquet_path = "/tmp/transient/tb_fat_sales_order_items"
...
expectativa_nulos = df_ge.expect_column_values_to_not_be_null(column='SALESORDERID')

#Outra coluna para verificação
expectativa_nulos_outra = df_ge.expect_column_values_to_not_be_null(column='PRODUCTID')
...
delta = "/tmp/bronze/tb_fat_sales_order_items"
df.write.partitionBy("dt_ingestion").format("delta").mode("overwrite").save(delta)

# COMMAND ----------

# MAGIC %fs ls dbfs:/tmp/bronze/tb_fat_sales_order_items
